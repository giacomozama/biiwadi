...
pub struct InhibitorState {
    event_queue: Option<wayland_client::EventQueue<Self>>,
...
impl Dispatch<wl_registry::WlRegistry, ()> for InhibitorState {
    fn event(
        state: &mut Self,
...
impl Dispatch<wl_compositor::WlCompositor, ()> for InhibitorState {
    fn event(
        _state: &mut Self,
...
impl Dispatch<wl_surface::WlSurface, ()> for InhibitorState {
    fn event(
        _state: &mut Self,
...
impl Dispatch<ZwpIdleInhibitManagerV1, ()> for InhibitorState {
    fn event(
        _state: &mut Self,
...
impl Dispatch<ZwpIdleInhibitorV1, ()> for InhibitorState {
    fn event(
        _state: &mut Self,
...
impl InhibitorState {
    pub fn setup(&mut self) {
        ...
        if let Some((inhibit_manager, _)) = &self.inhibit_manager {
            if let Some(surface) = &self.surface {
                self.inhibitor = Some(inhibit_manager.create_inhibitor(surface, &queue_handle, ()));
                event_queue.roundtrip(self).unwrap();
            }
        }
        ...
        println!("Inhibitor activated");
    }
}

impl Drop for InhibitorState {
    fn drop(&mut self) {
        if let Some((inhibit_manager, _)) = self.inhibit_manager.take() {
